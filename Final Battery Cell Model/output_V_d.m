function V = output_V_d(in)

persistent  n p a_p a_n c d
...
Q_loss = in(5); %film layer resistance
T = in(6); %reference cell temperature

delta_film_n = (d.M_p/(n.L_n*n.A_n*n.a_n*d.rho_p*c.F))*(Q_loss); %SEI layer thickness (Assume that initial film layer = 0 & initial Capacity Loss = 0)
R_film_n = d.R_sei + delta_film_n/d.K_p; %Resistance of the SEI layer
...
%output voltage   
V = U_ref_p - U_ref_n + eta_p - eta_n - ((p.R_film_p/(p.A_p*p.L_p*a_p))+(R_film_n/(n.A_n*n.L_n*a_n)) + c.R_c)*u;
